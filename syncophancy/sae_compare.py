...
def main():
    ...
    # --- feature-space route scatter (d_install vs d_anti), parallels feature_route.py ---
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        same = np.sign(install) == np.sign(suppress)
        fig, ax = plt.subplots(figsize=(5.6, 5.3))
        ax.axhline(0, c="0.8", lw=.7); ax.axvline(0, c="0.8", lw=.7)
        lim = max(np.abs(install).max(), np.abs(suppress).max()) * 1.05
        ax.plot([-lim, lim], [-lim, lim], "--", c="#27ae60", lw=1, label="same direction (cos>0)")
        ax.plot([-lim, lim], [lim, -lim], "--", c="0.6", lw=1, label="opposite (bidirectional)")
        ax.scatter(install[same], suppress[same], s=7, alpha=.3, c="#8e44ad", label="same sign")
        ax.scatter(install[~same], suppress[~same], s=7, alpha=.3, c="#7f8c9b", label="sign-flipped")
        ax.set_xlabel("d_install  (pro - base)"); ax.set_ylabel("d_anti  (anti - base)")
        ax.set_title(f"SAE feature route  .  cos = {cos:+.3f}")
        ax.legend(fontsize=8, frameon=False, loc="lower right")
        fig.tight_layout(); fig.savefig(args.out.replace(".json", "_scatter.png"), dpi=140, bbox_inches="tight")
        np.savez(args.out.replace(".json", "_deltas.npz"),
                 base=base, install=install, suppress=suppress)
        print("saved", args.out.replace(".json", "_scatter.png"), "and _deltas.npz")
